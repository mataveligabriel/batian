...
import { Plus, X, Wifi, WifiOff, Server, TerminalSquare } from "lucide-react";

function TerminalPane({ device, onClose, active }) {
  ...
  useEffect(() => {
    if (active && fitRef.current) {
      setTimeout(() => { try { fitRef.current.fit(); } catch { /* noop */ } }, 60);
    }
  }, [active]);

  return (
    <div className={`absolute inset-0 flex flex-col ${active ? "visible z-10" : "invisible z-0"}`} data-testid={`terminal-pane-${device.id}`}>
      <div className="flex items-center justify-between border-b border-[#1E293B] px-4 py-2 bg-[#0B111C]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[11px] font-mono text-slate-500">{device.host}:{device.port} · {device.username}</div>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <span data-testid={`terminal-status-${device.id}`} className={`text-[11px] font-mono uppercase tracking-widest ${
            ...
            "text-red-400"
          }`}>{status === "connected" ? "conectado" : status}</span>
        </div>
      </div>
...
export default function TerminalPage() {
  ...
  useEffect(() => {
    api.get("/devices").then(r => {
      ...
      if (deviceId) {
        ...
        if (d) openTab(d);
        else toast.error("Equipamento não encontrado");
      } else if (r.data.length === 0) {
        toast.info("Nenhum equipamento cadastrado ainda");
      }
    });
  ...
  const openTab = (d) => {
    setTabs(prev => prev.find(t => t.id === d.id) ? prev : [...prev, d]);
    setActive(d.id);
    setPickerOpen(false);
  };
  const closeTab = (id) => {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0" data-testid="terminal-page">
      <div className="px-6 py-4 border-b border-[#1E293B] bg-[#0B111C] flex items-center justify-between">
        ...
        <div className="text-xs font-mono text-slate-500">
          <span className="text-emerald-400">{tabs.length}</span> sessão(ões) ativa(s)
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 px-3 py-2 border-b border-[#1E293B] bg-[#0A0F19] overflow-x-auto">
        {tabs.map(t => (
          <div key={t.id}
            ...
            <X className="w-3.5 h-3.5 opacity-60 hover:opacity-100 hover:text-red-400"
        ...
        <div className="relative">
          ...
          {pickerOpen && (
            <div className="absolute top-full mt-1 left-0 z-20 w-72 bg-[#111722] border border-[#1E293B] rounded-md shadow-xl max-h-80 overflow-y-auto">
              {devices.length === 0 && <div className="p-4 text-sm text-slate-500 font-mono">Sem equipamentos</div>}
              {devices.map(d => (
                <div key={d.id} onClick={() => openTab(d)}
      ...
      <div className="flex-1 min-h-0 bg-[#05070A] relative">
        {tabs.length === 0 ? (
          ...
          tabs.map(t => <TerminalPane key={t.id} device={t} active={active === t.id} onClose={() => closeTab(t.id)} />)
        )}
      </div>
    </div>
  );
