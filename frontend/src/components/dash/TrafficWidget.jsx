  ...
  return (
    <div className="min-w-0">
      ...
      <div className="flex items-baseline gap-1.5 mt-0.5 min-w-0">
        <div className={`font-heading ${big ? "text-4xl" : "text-[1.6rem] leading-tight"} font-bold text-slate-50 tabular-nums whitespace-nowrap`} data-testid="hero-value">{fmtBps(value)}</div>
      ...
      <div className="text-[11px] font-mono text-slate-400 mt-1 truncate" title={`p95 ${fmtBps(p95)} · pico ${fmtBps(max)}`}>p95 <b className="text-slate-200">{fmtBps(p95)}</b> · pico <b className="text-slate-200">{fmtBps(max)}</b></div>
    </div>
  );
}
  ...
  return (
    <div data-testid={`traffic-widget-${widget.id}`}>
      ...
      <div className="grid grid-cols-2 gap-x-4 mb-2">
        <Hero icon={ArrowDownLeft} label="Entrada" color={IN.color} value={st.in.cur} pct={pct(st.in.cur)} p95={st.in.p95} max={st.in.max} big={height >= 360} />
